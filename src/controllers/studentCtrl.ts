...
import { getPerfectStudent } from "../utils/students";


// /students
export const getStudentsCtrl = async (req: Request, res: Response, next: NextFunction) => {
    const students = await getStudents()
    const perfectStudents = await Promise.all(students.map(async student => await getPerfectStudent(student)))
    const pathname = req.url.replace('/', '')
    res.render('pages/students/students', {students: perfectStudents, pathname})
}
...
export const createStudentCtrl = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if(req.method === 'GET') {
        ...
        res.render('pages/students/add_student', {sectors, semesters})
    }
...
export const updateStudentCtrl = async (req: Request, res: Response, next: NextFunction) => {
    if(req.method === 'GET') {
        ...
        const sectors = await getSectors()
        const semesters = await getSemesters()
        res.render('pages/students/update_student', {student, birthDate, sectors, semesters})
    }
    else if(req.method === 'POST') {
        console.log(req.body)
